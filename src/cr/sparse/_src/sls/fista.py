...
def fista(
    ...
    def init():
        ...
        return FISTAState(x=x0, z=x0, r=r, t=1., r_norm_sqr=r_norm_sqr,
            x_change_norm=1e10, 
            iterations=0)
    ...
    def body(state):
        ...
        x = basis.times(alpha)
        # update t
        t = (1. + jnp.sqrt(1. + 4. * state.t ** 2)) / 2.
        # update z
        z = x + ((state.t - 1.) / t) * (x - state.x)
        # update the residual
        r = b - operator.times(z)
        # compute the norm of the current residual
        r_norm_sqr = arr_l2norm_sqr(r)
        # compute the norm of change in x
        x_change_norm = arr_l2norm(x - state.x)
        return FISTAState(x=x, z=z, r=r, t=t, r_norm_sqr=r_norm_sqr, 
