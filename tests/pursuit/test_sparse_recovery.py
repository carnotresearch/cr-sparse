...
def test_cosamp():
    sol = cosamp.solve_jit(Phi, y, K)
    rp = RecoveryPerformance(Phi, y, x, sol=sol)
    assert rp.success


def test_sp():
    sol = sp.solve_jit(Phi, y, K)
    rp = RecoveryPerformance(Phi, y, x, sol=sol)
    assert rp.success
